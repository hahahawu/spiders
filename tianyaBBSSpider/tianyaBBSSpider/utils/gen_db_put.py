...
from tianyaBBSSpider.utils import MD5Utils
...
def gen_post_put(post_item):
    ...
    row_key = gen_row_key(MD5Utils.md5_code(author_id), MD5Utils.md5_code(post_id))
...
def gen_fans_put(fans_item):
    ...
    for key in fans_item:
        column_values.append(TColumnValue(info_fml, key.encode(), str(fans_item[key]).encode()))
