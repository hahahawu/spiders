...
from sanqinSpider.utils.dateTimeFormator import dataTimeFormator
...
def get_news_info(response):
    ...
    # news_id
    news_id = ''.join(url.split('/')[-3::]).split('.')[0] + '_sanqin'
    ...
    item['publish_time'] = check(dataTimeFormator(date_time))
    ...
    item['content'] = str_formator(content)
...
def get_dark_pic_item(response):
    ...
    item['publish_time'] = check(dataTimeFormator(date))
    ...
    item['content'] = str_formator(check(';'.join(content_text)))
