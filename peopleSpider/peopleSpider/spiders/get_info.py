...
from peopleSpider.utils.dealStr import dealStr
from peopleSpider.utils.timeFormat1 import timeFormat1
...
def get_news_1_info(response):
    ...
    if date_time_div:
        ...
        date_time=timeFormat1(check_value(date_time))
    else:
        date_time = ''
    ...
    item['pre_title'] = dealStr(check_value(pre_title))
    item['title'] = dealStr(check_value(title))
    item['sub_title'] = dealStr(check_value(sub_title))
...
def get_news_2_info(response):
    ...
    item['date_time'] = timeFormat1(check_value(date_time))
...
def get_pic_news_info(response):
    ...
    if path_div:
        for x in path_div[1:]:
            href = x.xpath('./@href').extract_first()
            if href:
                path_href = path_href + href + '; '

            text = x.xpath('string(.)').extract_first()
            if text:
                path_text = path_text + text + '; '
    ...
    if date_time:
        date_time = timeFormat1(date_time.strip().split('\xa0')[-1])
    else:
        date_time = []
    ...
    # 关键字
    key_words = sel.xpath('//meta[@name="keywords"]/@content').extract_first()
    item['key_words'] = dealStr(key_words)
    ...
    # 内容
    content_text = dealStr(get_pic_content(response))
    item['content'] = []
    if content_text:
...
def get_dang_news_info(response):
    ...
    item['source_text'] = dealStr(check_value(source_text))
    item['source_href'] = check_value(source_href)
    ...
    item['pre_title'] = dealStr(pre_title)

    sub_title = ''
    item['sub_title'] = dealStr(sub_title)

    # 内容和图片
    content_text, content_img_urls, content_urls = get_dang_content_div(response)
    item['content'] = dealStr(check_value(content_text))
...
def get_fanfu_info(response):
    ...
    if date_time:
        item['date_time'] = timeFormat1(date_time.strip().split('\xa0')[0])
    else:
        item['date_time'] = ''
...
def get_dangshi_info(response):
    ...
    if source_div:
        ...
        if source_text == '手机看新闻':
            ...
            item['source_text'] = dealStr(check_value(source_text.strip()))
            source_href = source_div.xpath('./@href').extract_first()
    ...
    item['content'] = dealStr(check_value(content))
...
def get_dangshi_2_info(response):
    ...
    if date_time:
        date_time = date_time.strip().split('\xa0')[0]
        item['date_time'] = timeFormat1(date_time)
    else:
        item['date_time'] = ''
...
def get_qunzhong_info(response):
    ...
    # 内容
    item['content'] = dealStr(get_qunzhong_content(response))
