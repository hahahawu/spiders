...
#
# from xinhuaNewsSpider.utils.gen_db_put import *
#
#
...
from xinhuaNewsSpider.settings import TB_NEWS
from xinhuaNewsSpider.utils.dateTimeFormator import timeFormat

str_formator = lambda x: x.replace('\u3000', ' ').replace('\xa0', '').replace('\r\n', '')
...
class SaveToMysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        '''
            ...
            id去除中括号
        '''
        item['content'] = str_formator(item['content'])
        item['source'] = str_formator(item['source'])
        item['editor'] = str_formator(item['editor'])
        item['date_time'] = timeFormat(item['date_time'])
