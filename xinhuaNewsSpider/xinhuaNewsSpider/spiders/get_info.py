...
def get_news_1_info(response):
    ...
    news_item['article_id'] = ''.join(response.url.split('/')[-3:]).split('.')[0] + '_xinhua_news'
    ...
    if not path_div:
        ...
        path_div = sel.xpath('//div[@class="crumbs domPC clearfix"]/span[@class="curDiv clearfix"]/*/a')
    path_text_list = []
    path_href_list = []
...
def get_pic_news_info(response):
    ...
    temp_source_div = sel.xpath('//em[@id="source"]').xpath("string(.)").extract_first()
    if temp_source_div:
        source_div = temp_source_div
...
def get_hb_news_div(response):
    ...
    # path_text and path_href

    # title
    title = sel.xpath('//div[@class="nr_bt"]').extract_first()
    news_item['title'] = check_value(title)

    # datetime and source
    datetime = sel.xpath('//div[@class="nr_ly"]/text()').extract_first()
    news_item['date_time'] = check_value(datetime)
    source = sel.xpath('//div[@class="nr_ly"]/em/text()').extract_first()
    news_item['source'] = check_value(source)

    # content
    content = sel.xpath('//div[@id="nr_wz"]/p').xpath('string(.)').extract()
    news_item['content'] = check_value(''.join(content[:-1]))

    # picture_urls
    picture_urls = sel.xpath('//*[@id="nr_wz"]//img/@src').extract()
    news_item['picture_urls'] = [response.urljoin(pic_url) for pic_url in picture_urls if pic_url]

    # editor
    editor = sel.xpath('//div[@id="nr_wz"]/p').xpath('string(.)').extract()[-1]
    news_item['editor'] = check_value(editor)
...
def get_p_detail_div(response):
    ...
    # path_text and path_href
    path_div = sel.xpath('//div[@class="news-position"]/a')
    path_text_list = []
    path_href_list = []
    for path in path_div:
        path_text = path.xpath('string(.)').extract_first()
    ...
    # title
    title = sel.xpath('//div[@class="h-title"]').xpath("string(.)").extract_first()
    news_item['title'] = check_value(title)

    # datetime and source
    datetime = sel.xpath('//*[@class="h-time"]/text()').extract_first()
    news_item['date_time'] = check_value(datetime)
    source = sel.xpath('//em[@id="source"]').xpath("string(.)").extract_first()
    news_item['source'] = check_value(source)

    # content
    content = sel.xpath('//div[@id="p-detail"]/p').xpath('string(.)').extract()
    news_item['content'] = check_value(''.join(content[:-1]))

    # picture_urls
    picture_urls = sel.xpath('//*[@id="p-detail"]//img/@src').extract()
    news_item['picture_urls'] = [response.urljoin(pic_url) for pic_url in picture_urls if pic_url]

    # editor
    editor = sel.xpath('//div[@id="p-detail"]/div[@class="p-tags"]/span[@class="p-jc"]/text()').extract_first()
    news_item['editor'] = check_value(editor)
